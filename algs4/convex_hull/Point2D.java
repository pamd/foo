...
public class Point2D implements Comparable<Point2D> {
     ...
     */
    // dhu: This function is called by GrahamScan.java
	...
	// If p1 and p2 have same y coordinates, but p1.x < p2.x, 
	// then arbitrarily set p1 < p2
        if (this.x < that.x) 
	    return -1;
        if (this.x > that.x) 
	    return +1;

	// Otherwise these two points must be overlapping.
        return 0;
    }
    ...
    private class PolarOrder implements Comparator<Point2D> {
	    ...
	    // 3-collinear and horizontal
            if (dy1 == 0 && dy2 == 0) {  
		// If q1 is on right side, q2 on left side, then q1 < q2
                if (dx1 >= 0 && dx2 < 0) 
		    return -1;

		// q1 is on left side, q2 on right side, then q1 > q2
                if (dx2 >= 0 && dx1 < 0) 
		    return +1;
	    ...
	    // q1 and q2 are both above or below.
	    // dhu: In this implementation, if Point2D.this, q1 and q2 
	    // are collinear, then q1 and q2 are equal. 
	    // This causes a bug in GrahamScan.java.
