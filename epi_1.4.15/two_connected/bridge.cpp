/* Variant 16.4.1 Part 2: Let G be a connected graph. An edge of G is a bridge 
 * if its removal disconnects G. Find all bridge edges in G. 
 * 
 ...
 * (Locally saved as: bridge_algs4.java)
 */
...
class Graph {
  ...
  void bridge_util(int v, bool visited[], int discovery[], int low[], int parent[]);
public:
...
void Graph::bridge_util(int u, bool visited[], int discovery[], 
		       int low[], int parent[]) {
  ...
  for (i = adj[u].begin(); i != adj[u].end(); ++i) {
    ...
    if (!visited[v]) {
      parent[v] = u;
      bridge_util(v, visited, disc, low, parent);
