...
// Multiple solutions from anson627
...
public:
  ...
  // pre-order walk with looking up min and max, takes O(n*log(n)) time
  ...
  // pre-order walk with updating min and max, takes O(n) time
  ...
  // in-order walk and check if it is no-decreasing, takes O(n) time
