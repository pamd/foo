...
public class DirectedEulerianCycle {
   ...
   public DirectedEulerianCycle(Digraph G) {
      ...
      // Find Eulerian tour (cycle)
      stack.push(0);
      while (!stack.isEmpty()) {
         int s = stack.pop();
         tour.push(s);
         int v = s;
         // If the vertex that was just popped out of stack still has outbound edges,
      ...
      // Check whether all edges have been visited.
      // If the input graph is not connected, then at least one edge has not been visited,
      // so the graph must NOT have Eulerian cycle.
      for (int v = 0; v < G.V(); v++) {
         if (!adj[v].isEmpty()) {
            isEulerian = false;
         }
   ...
   // Return Eulerian tour, or null (if no such tour exists).
